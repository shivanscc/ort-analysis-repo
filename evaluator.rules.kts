...
val noneRiskLicenses = getLicensesForCategory("none-risk")
val criticalRiskLicenses = getLicensesForCategory("critical-risk")
val highRiskLicenses = getLicensesForCategory("high-risk")
val mediumRiskLicenses = getLicensesForCategory("medium-risk")
val lowRiskLicenses = getLicensesForCategory("low-risk")
...
).map { SpdxSingleLicenseExpression.parse(it) }.toSet() + noneRiskLicenses
...
fun PackageRule.LicenseRule.isNoneRisk()=
    object : RuleMatcher {
        override val description = "isNoneRisk($license)"

        override fun matches() = license in noneRiskLicenses
    }

fun PackageRule.LicenseRule.isCriticalRisk()=
    object : RuleMatcher {
        override val description = "isCriticalRisk($license)"

        override fun matches() = license in criticalRiskLicenses
    }

fun PackageRule.LicenseRule.isHighRisk()=
    object : RuleMatcher {
        override val description = "isHighRisk($license)"

        override fun matches() = license in highRiskLicenses
    }

fun PackageRule.LicenseRule.isMediumRisk()=
    object : RuleMatcher {
        override val description = "isMediumRisk($license)"

        override fun matches() = license in mediumRiskLicenses
    }

fun PackageRule.LicenseRule.isLowRisk()=
    object : RuleMatcher {
        override val description = "isLowRisk($license)"

        override fun matches() = license in lowRiskLicenses
    }
...
fun RuleSet.criticalRiskInDependencyRule() = packageRule("CRITICAL_RISK_IN_DEPENDENCY") {
    require {
        -isProject()
        -isExcluded()
    }

    licenseRule("CRITICAL_RISK_IN_DEPENDENCY", LicenseView.CONCLUDED_OR_DECLARED_AND_DETECTED) {
        require {
            +isCriticalRisk()
            -isExcluded()
        }
        ...
        error(
            "The dependency '${pkg.metadata.id.toCoordinates()}' is licensed under the ScanCode 'critical-risk' " +
                    "categorized license $license. This is CRITICAL and requires immediate blocking and urgent approval.",
            howToFixLicenseViolationDefault(license.toString(), licenseSource)
        )
    }
}

fun RuleSet.highRiskInDependencyRule() = packageRule("HIGH_RISK_IN_DEPENDENCY") {
    require {
        -isProject()
        -isExcluded()
    }

    licenseRule("HIGH_RISK_IN_DEPENDENCY", LicenseView.CONCLUDED_OR_DECLARED_AND_DETECTED) {
        require {
            +isHighRisk()
            -isExcluded()
        }
        ...
        error(
            "The dependency '${pkg.metadata.id.toCoordinates()}' is licensed under the ScanCode 'high-risk' " +
                    "categorized license $license. This requires immediate attention and approval.",
            howToFixLicenseViolationDefault(license.toString(), licenseSource)
        )
    }
}

fun RuleSet.mediumRiskInDependencyRule() = packageRule("MEDIUM_RISK_IN_DEPENDENCY") {
    require {
        -isProject()
        -isExcluded()
    }

    licenseRule("MEDIUM_RISK_IN_DEPENDENCY", LicenseView.CONCLUDED_OR_DECLARED_AND_DETECTED) {
        require {
            +isMediumRisk()
            -isExcluded()
        }
        ...
        warning(
            "The dependency '${pkg.metadata.id.toCoordinates()}' is licensed under the ScanCode 'medium-risk' " +
                    "categorized license $license. This requires review and approval.",
            howToFixLicenseViolationDefault(license.toString(), licenseSource)
        )
    }
}

fun RuleSet.lowRiskInDependencyRule() = packageRule("LOW_RISK_IN_DEPENDENCY") {
    require {
        -isProject()
        -isExcluded()
    }

    licenseRule("LOW_RISK_IN_DEPENDENCY", LicenseView.CONCLUDED_OR_DECLARED_AND_DETECTED) {
        require {
            +isLowRisk()
            -isExcluded()
        }
        ...
        hint(
            "The dependency '${pkg.metadata.id.toCoordinates()}' is licensed under the ScanCode 'low-risk' " +
                    "categorized license $license. This is informational only.",
            howToFixLicenseViolationDefault(license.toString(), licenseSource)
        )
    }
}
...
fun RuleSet.commonRules() {
    ...
    criticalRiskInDependencyRule()
    highRiskInDependencyRule()
    mediumRiskInDependencyRule()
    lowRiskInDependencyRule()
